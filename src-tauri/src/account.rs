...
mod tests {
    ...
    async fn login_watchdog_kills_hanging_cli() {
        ...
        // GitHub-hosted /tmp is often noexec; write the fake CLI under target/.
        let dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("target")
            .join(format!(
                "supercharge-login-timeout-test-{}",
                std::process::id()
            ));
        let _ = std::fs::create_dir_all(&dir);
        let fake = dir.join("supercharge");
        {
            ...
            writeln!(
                f,
                "#!/bin/sh\nif [ \"$1\" = \"--version\" ]; then echo \"supercharge 1.3.15\"; exit 0; fi\nsleep 600"
            )
            .unwrap();
        }
