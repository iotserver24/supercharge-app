//! List Supercharge CLI-tracked worktrees (`grok worktree list`).
//!
...
///
/// Columns observed (Supercharge 0.2.x):
...
// ── CLI worktree DB (path / stats / rebuild) — Supercharge 0.2.117+ ───────────
...
///
/// Observed shape (Supercharge 0.2.117):
/// ```text
...
fn list_cli_worktrees_blocking(
    ...
    if !probe.found {
        return CliWorktreesResult {
            available: false,
            worktrees: vec![],
            reason: Some("Supercharge CLI not found".into()),
            cli_found: false,
            source: Some("none".into()),
        };
    }
...
fn cli_worktree_db_path_blocking(home: PathBuf) -> CliWorktreeDbPathResult {
    ...
    if !cli_found {
        return soft_db_path_err(false, false, "Supercharge CLI not found");
    }
        ...
        Ok((stdout, stderr, ok)) => {
            if looks_like_unsupported_worktree_db(&stderr, &stdout) {
                return soft_db_path_err(
                    true,
                    true,
                    "CLI worktree DB requires Supercharge CLI 0.2.117+",
                );
            }
...
fn cli_worktree_db_stats_blocking() -> CliWorktreeDbStatsResult {
    let (cli_found, _) = db_cli_probe();
    if !cli_found {
        return soft_db_stats_err(false, false, "Supercharge CLI not found");
    }
...
fn cli_worktree_db_rebuild_blocking() -> CliWorktreeDbRebuildResult {
    let (cli_found, _) = db_cli_probe();
    if !cli_found {
        return soft_db_rebuild_err(false, false, "Supercharge CLI not found");
    }
