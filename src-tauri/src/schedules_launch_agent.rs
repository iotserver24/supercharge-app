...
//! installs a user LaunchAgent that starts the **full Supercharge App** at login and
...
"#
    ...
    format!(
        r#"#!/bin/bash
# Generated by Supercharge App — schedules keep-alive helper.
# NOT a headless daemon. Starts the full Supercharge App so automation_runner can tick
...
"#
    ...
    format!(
        r#"#!/bin/bash
# Generated by Supercharge App — one-shot schedule fire helper (AUTO-HEADLESS A2).
# NOT a KeepAlive daemon and NOT a continuous scheduler.
# Boots Supercharge App with {flag}: fires at most one due scheduled task, then exits.
...
<!-- Generated by Supercharge App. Label: {label}
...
fn limits_text(app_path: &Path, helper: &Path) -> String {
    format!(
        "Supercharge App — schedules LaunchAgent + one-shot helpers\n\
         ==================================================\n\
         \n\
         This is NOT a headless background daemon and NOT a separate scheduler.\n\
         \n\
         Full-app LaunchAgent (optional, install via UI):\n\
         - At login (RunAtLoad): starts the full Supercharge App (prefer tray).\n\
