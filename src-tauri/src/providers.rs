...
/// Whether Use/Save should also update the terminal CLI home (`~/.supercharge`).
/// Isolated app-home tests must not touch the real CLI profile unless they set
/// `SUPERCHARGE_HOME` themselves.
fn should_mirror_to_shared_cli() -> bool {
    let shared = crate::paths::shared_supercharge_home();
    if shared == agent_home_dir() {
        return false;
    }
    let isolated_app_home = std::env::var_os("SUPERCHARGE_APP_HOME")
        .filter(|v| !v.is_empty())
        .or_else(|| std::env::var_os("GROK_APP_HOME").filter(|v| !v.is_empty()))
        .is_some();
    let explicit_cli_home = std::env::var_os("SUPERCHARGE_HOME")
        .filter(|v| !v.is_empty())
        .is_some();
    !isolated_app_home || explicit_cli_home
}

/// Stable field order for mirroring a `[model.*]` section (known keys first).
fn section_fields_for_mirror(section: &ModelSection) -> Vec<(String, String)> {
    const PREFERRED: &[&str] = &[
        "model",
        "base_url",
        "name",
    ...
    for key in PREFERRED {
        if let Some(v) = section.fields.get(*key).filter(|s| !s.trim().is_empty()) {
            fields.push(((*key).to_string(), v.clone()));
            used.insert(*key);
        }
    }
    let mut rest: Vec<_> = section
        .fields
        .iter()
        .filter(|(k, v)| !used.contains(k.as_str()) && !v.trim().is_empty())
        .collect();
    rest.sort_by(|a, b| a.0.cmp(b.0));
    for (k, v) in rest {
        fields.push((k.clone(), v.clone()));
    }
    if !fields.iter().any(|(k, _)| k == "auth_scheme") {
        fields.push(("auth_scheme".into(), "bearer".into()));
    }
    if !fields.iter().any(|(k, _)| k == "context_window") {
        fields.push(("context_window".into(), "200000".into()));
    }
    fields
}

fn durable_section_importable(section: &ModelSection) -> bool {
    if !is_custom(&section.fields) {
        return false;
    }
    let key = section
    ...
    let name = section.fields.get("name").map(|s| s.trim()).unwrap_or("");
    if name.eq_ignore_ascii_case("xAI Official") {
        return false;
    }
    // Supercharge CLI catalog rows (`model_provider = "setup"`) stay in
    // ~/.supercharge; only App-saved channels (or CLI rows without that tag)
    // hydrate the Providers list.
    let cli_catalog = section
        .fields
        .get("model_provider")
        .map(|s| s.trim() == "setup")
        .unwrap_or(false);
    let app_owned = section.fields.contains_key(APP_PROVIDER_MODE_KEY)
        || section.fields.contains_key(APP_MODELS_KEY);
    !cli_catalog || app_owned
}

/// Copy missing custom `[model.*]` blocks from `src` into `dest` (no overwrite).
fn merge_missing_custom_sections(dest: &str, src: &str) -> String {
    let existing: std::collections::HashSet<String> = parse_model_sections(dest)
        .into_iter()
        .map(|s| s.id)
        .collect();
    let mut text = dest.to_string();
    for section in parse_model_sections(src) {
        if existing.contains(&section.id) || !durable_section_importable(&section) {
            continue;
        }
        text = append_section(&text, &section.id, &section_fields_for_mirror(&section));
    }
    text
}

fn restore_default_if_missing(dest: &str, src: &str) -> String {
    if get_models_default(dest).is_some() {
        return dest.to_string();
    }
    let Some(d) = get_models_default(src) else {
        return dest.to_string();
    };
    if parse_model_sections(dest).iter().any(|s| s.id == d) {
        return set_models_default(dest, &d);
    }
    dest.to_string()
}

/// Re-import custom providers from heal backups + `~/.supercharge` when the
/// App agent-home copy is missing them (updates / heals / reinstalls).
fn hydrate_agent_home_from_durable_stores() -> Result<bool, String> {
    if !should_mirror_to_shared_cli() {
        return Ok(false);
    }
    ...
    if let Ok(rd) = fs::read_dir(&home) {
        let mut baks: Vec<PathBuf> = rd
            ...
            .filter(|p| {
                p.file_name()
                    .and_then(|n| n.to_str())
                    .is_some_and(|n| n.starts_with("config.toml.bak-heal-"))
            })
            .collect();
        baks.sort();
        baks.reverse();
        for bak in baks {
            let src = read_text(&bak);
            text = merge_missing_custom_sections(&text, &src);
            text = restore_default_if_missing(&text, &src);
        }
    }

    let shared = crate::paths::shared_supercharge_home().join("config.toml");
    if shared.is_file() {
        let src = read_text(&shared);
        text = merge_missing_custom_sections(&text, &src);
        text = restore_default_if_missing(&text, &src);
    }
    ...
    tracing::info!(
        target: "providers",
        "restored missing custom providers from ~/.supercharge / heal backups"
    );
    Ok(true)
}

/// Copy an App-managed `[model.<id>]` into the Supercharge CLI config and,
/// when activating, set `[models].default` so `supercharge` at the repo root
/// uses the same route the desktop Use button selected.
fn mirror_provider_to_shared_cli(id: &str, set_default: bool) -> Result<(), String> {
    if !should_mirror_to_shared_cli() {
        return Ok(());
    }
    let id = id.trim();
    ...
    let src = read_text(&agent_config_toml());
    let Some(section) = parse_model_sections(&src).into_iter().find(|s| s.id == id) else {
        return Ok(());
    };
    let shared_home = crate::paths::shared_supercharge_home();
    fs::create_dir_all(&shared_home).map_err(|e| e.to_string())?;
    let dest = shared_home.join("config.toml");
    let mut text = read_text(&dest);
    let key = section.fields.get("api_key").cloned().unwrap_or_default();
    if key.trim().is_empty() {
        return Ok(());
    }
    let fields = section_fields_for_mirror(&section);
...
pub fn list_custom_providers() -> Result<ProvidersListResult, String> {
    let home = ensure_agent_home()?;
    let path = agent_config_toml();
    let _ = hydrate_agent_home_from_durable_stores();
...
pub fn activate_provider(
    ...
    match source.as_str() {
        ...
        "custom" => {
            ...
            let _ = mirror_provider_to_shared_cli(id, true);
            Ok(result)
        }
...
pub fn upsert_custom_provider(input: UpsertProviderInput) -> Result<ProvidersListResult, String> {
    ...
    let context_window = resolved_context_window.unwrap_or(200_000);
    fields.push(("context_window".into(), context_window.to_string()));
    // Supercharge CLI `[model.*]` entries expect a bearer scheme when an
    // inline api_key is present; omitting it leaves the section half-parsed.
    if !fields.iter().any(|(k, _)| k == "auth_scheme") {
        fields.push(("auth_scheme".into(), "bearer".into()));
    }
    ...
    let _ = mirror_provider_to_shared_cli(&id, input.set_as_default.unwrap_or(false));
    Ok(result)
}
...
mod tests {
    ...
    #[test]
    fn merge_missing_custom_sections_restores_app_models() {
        let dest = "[models]\nmax_retries = 12\n";
        let src = r#"
[models]
...
app_models = "[{\"id\":\"openai/gpt-5.6-luna\",\"name\":\"openai/gpt-5.6-luna\"}]"
"#;
        let merged = merge_missing_custom_sections(dest, src);
        assert!(merged.contains("[model.pollinations]"), "{merged}");
        assert!(merged.contains("app_models"), "{merged}");
        assert!(merged.contains("sk_test"), "{merged}");
        let with_default = restore_default_if_missing(&merged, src);
        assert!(
            with_default.contains("default = \"pollinations\""),
            "{with_default}"
        );
        let again = merge_missing_custom_sections(&with_default, src);
        assert_eq!(
            parse_model_sections(&again)
                .iter()
                .filter(|s| s.id == "pollinations")
                .count(),
            1
        );
        let cli_catalog = r#"
...
name = "gpt-6-astra"
api_key = "azure-key"
model_provider = "setup"
"#;
        let skipped = merge_missing_custom_sections(&again, cli_catalog);
        assert!(!skipped.contains("[model.gpt-6-astra]"), "{skipped}");
    }
...
"#;
    ...
    #[test]
    fn use_mirrors_provider_into_cli_home_and_sets_default() {
        let _lock = crate::paths::APP_HOME_ENV_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let stamp = uuid::Uuid::new_v4();
        let app_home = std::env::temp_dir().join(format!("grok-app-use-app-{stamp}"));
        let cli_home = std::env::temp_dir().join(format!("grok-app-use-cli-{stamp}"));
        let prev_app = std::env::var("GROK_APP_HOME").ok();
        let prev_cli = std::env::var("SUPERCHARGE_HOME").ok();
        std::env::set_var("GROK_APP_HOME", &app_home);
        std::env::set_var("SUPERCHARGE_HOME", &cli_home);

        upsert_custom_provider(UpsertProviderInput {
            id: "pollinations".into(),
            model: "openai".into(),
            base_url: "https://gen.pollinations.ai/v1".into(),
            name: Some("Pollinations".into()),
            api_key: Some("sk-test-use".into()),
            api_backend: Some("chat_completions".into()),
            provider_mode: Some(PROVIDER_MODE_GENERIC.into()),
            set_as_default: Some(false),
            create_only: Some(true),
        ...
        .expect("upsert");
        let listed = activate_provider("custom", Some("pollinations")).expect("use");
        assert_eq!(listed.active_source, "custom");
        assert_eq!(listed.active_provider_id.as_deref(), Some("pollinations"));

        let cli = std::fs::read_to_string(cli_home.join("config.toml")).expect("cli config");
        assert!(cli.contains("[model.pollinations]"), "{cli}");
        assert!(cli.contains("default = \"pollinations\""), "{cli}");
        assert!(cli.contains("auth_scheme = \"bearer\""), "{cli}");
        assert!(cli.contains("context_window = 200000"), "{cli}");
        ...
        match prev_cli {
            Some(value) => std::env::set_var("SUPERCHARGE_HOME", value),
            None => std::env::remove_var("SUPERCHARGE_HOME"),
        }
