...
// Some host modules are kept for mergeability but Supercharge App does not
// call those surfaces, so they trip `dead_code` under `clippy -D warnings`.
// Keep the modules; silence the unused-item lint at the crate root.
#![allow(dead_code)]
