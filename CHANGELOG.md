...
> **Highlight:** Pin chats globally, copy commands and diffs, keep Shared session data, and persist custom providers.
>
> **中文 · 亮点：** 置顶对话、复制命令和 diff、共享会话数据会记住，自定义渠道会写进 ~/.supercharge。

### Added
- Custom providers persist into `~/.supercharge` and restore after app updates or reinstalls.
...
### Fixed
- Shared session data mode is remembered, unless a custom provider needs independent. Independent still uses the app data folder, not always ~/.supercharge-app (#1228).
...
- The Windows desktop pet no longer appears as a second Supercharge taskbar icon.
...
**中文 · 修复**
- 选共享后会记住；只有自定义渠道必须独立时才保持独立。独立模式用应用数据目录，不一定是 ~/.supercharge-app（#1228）。
