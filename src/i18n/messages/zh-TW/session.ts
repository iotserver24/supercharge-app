...
export const zhTWSession = {
  ...
  "session.rulesHint": "僅本對話以 supercharge --rules 傳入。最多 {n} 個字元。儲存後會新開一輪 agent，規則才會生效；清空即清除。",
  ...
  "session.maxTurnsHint": "僅本對話以 supercharge --max-turns N 傳入（1–{max}）。留空或 0 繼承設定（目前：{global}）。儲存後 live Agent 會 soft-respawn。",
  ...
  "session.sysPromptHint": "僅本對話以 supercharge --system-prompt-override 傳入。取代預設系統提示詞（不是附加）。最多 {n} 個字元。儲存後 live Agent 會 soft-respawn；清空即清除。",
  ...
  "session.worktreeLayoutCli": "CLI worktrees 目錄（~/.supercharge/worktrees）",
  ...
  "session.forkCliSession": "分叉 CLI Agent 工作階段（新 agent id — supercharge --fork-session）",
  ...
  "session.exportMdHint": "選擇要包含的內容，然後下載 .md 檔或複製到剪貼簿。完整匯出在已關聯 agent 對話時優先用 CLI supercharge export（失敗則回退本機會話紀錄）。",
  ...
  "session.exportTraceUploadMessage": "將執行不帶 --local 的 supercharge trace，CLI 可能透過網路將對話封存上傳至 provider。僅在你確實要分享此對話 trace 以供支援或除錯時繼續。",
  ...
  "session.exportTraceNoCli": "找不到 Supercharge CLI。請先安裝或在設定中設定 CLI 路徑後再試。",
  "session.exportTraceTimeout": "等待 supercharge trace 逾時。可重試，或改用「僅本機匯出」以更快產生封存。",
  ...
  "session.exportDoneCli": "已透過 CLI（supercharge export）匯出對話",
