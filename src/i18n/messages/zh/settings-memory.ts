...
export const zhSettingsMemory = {
  ...
  "settings.memoryBrowserDesc": "浏览并搜索当前项目在磁盘上的 Supercharge CLI 记忆（{SUPERCHARGE_HOME}/memory）。正文搜索有上限；预览与摘录会脱敏疑似 API Key。可打开或在文件管理器中显示。不会改动应用内聊天记录。",
  ...
  "settings.memoryBrowser.emptyHint": "代理写入 MEMORY.md、会话摘要或索引到 {SUPERCHARGE_HOME}/memory 后会出现在此列表。",
  ...
  "settings.memoryBrowser.deleteConfirmMsg": "从 Supercharge CLI 记忆库删除「{name}」？此操作不可撤销。",
  ...
  "settings.memoryBrowser.hybridUnavailableHint": "已为 agent 工具设置 embedding 模型，但应用内浏览器没有 host 混合检索 CLI（supercharge memory search）— 仅关键词扫描。不会伪造 embedding。",
  ...
  "settings.memoryEmbedDesc": "从当前 SUPERCHARGE_HOME 的 config.toml 读取白名单 `[memory.*]` 键（embedding、search、MMR、dream、watcher）。缺失键保持未设置。独立 agent-home 可写入安全键并 soft-respawn；共享模式只读。应用内浏览器搜索始终为关键词，不会在客户端发明 embedding。",
  ...
  "settings.memoryEmbed.sharedWarning": "共享会话模式只读探测 ~/.supercharge/config.toml。请切换到「独立」会话数据模式以写入 agent-home 记忆键。",
  "settings.memoryEmbed.mode.independent": "独立（agent-home）",
  "settings.memoryEmbed.mode.shared": "共享（~/.supercharge）",
  ...
  "settings.memoryEmbed.watcherDesc": "监视 SUPERCHARGE_HOME/memory 外部编辑，搜索时重建索引。",
  ...
  "settings.memoryOps.hybridUnavailableHint": "已为 agent 工具设置 embedding 模型，但应用内浏览器没有 host 混合检索 CLI（supercharge memory search）— 仅关键词扫描。不会伪造 embedding。",
  ...
  "settings.memoryOps.clear.confirmMsg.workspace": "对当前项目执行 supercharge memory clear --workspace（MEMORY.md、会话摘要、索引）。不会删除应用内聊天记录。此操作不可撤销。",
  "settings.memoryOps.clear.confirmMsg.global": "在当前 SUPERCHARGE_HOME 下执行 supercharge memory clear --global（全局 MEMORY.md）。工作区项目记忆保留。不会删除应用内聊天记录。此操作不可撤销。",
  "settings.memoryOps.clear.confirmMsg.session": "应用 host 尚不支持仅清除会话范围（CLI 无 session 作用域）。",
  "settings.memoryOps.clear.confirmMsg.all": "在当前 SUPERCHARGE_HOME 下执行 supercharge memory clear --all，将删除全局与工作区记忆文件。不会删除应用内聊天记录。此操作不可撤销。",
