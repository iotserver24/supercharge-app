...
export const zhSession = {
  ...
  "session.rulesHint": "仅本会话以 supercharge --rules 传入。最多 {n} 个字符。保存后会新开一轮 agent，规则才会生效；清空即清除。",
  ...
  "session.maxTurnsHint": "仅本会话以 supercharge --max-turns N 传入（1–{max}）。留空或 0 继承设置（当前：{global}）。保存后 live Agent 会 soft-respawn。",
  ...
  "session.sysPromptHint": "仅本会话以 supercharge --system-prompt-override 传入。替换默认系统提示词（不是追加）。最多 {n} 个字符。保存后 live Agent 会 soft-respawn；清空即清除。",
  ...
  "session.worktreeLayoutCli": "CLI worktrees 目录（~/.supercharge/worktrees）",
  ...
  "session.forkCliSession": "分叉 CLI Agent 会话（新 agent id — supercharge --fork-session）",
  ...
  "session.exportMdHint": "选择要包含的内容，然后下载 .md 文件或复制到剪贴板。完整导出在已关联 agent 会话时优先用 CLI supercharge export（失败则回退本地会话记录）。",
  ...
  "session.exportTraceUploadMessage": "将运行不带 --local 的 supercharge trace，CLI 可能通过网络将会话归档上传到 provider。仅在你确实要分享该会话 trace 用于支持或调试时继续。",
  ...
  "session.exportTraceNoCli": "未找到 Supercharge CLI。请先安装或在设置中配置 CLI 路径后再试。",
  "session.exportTraceTimeout": "等待 supercharge trace 超时。可重试，或改用「仅本地导出」以更快生成归档。",
  ...
  "session.exportDoneCli": "已通过 CLI（supercharge export）导出会话",
