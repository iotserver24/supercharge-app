...
describe("wallpaper theme contrast CSS", () => {
  ...
  it("nests the workspace chip as a rounded rect inside a uniform inset", () => {
    expect(composerChromeCss).toMatch(
      /\.composer__chip-shell\s*\{[^}]*--composer-context-pad:\s*2px;[^}]*--composer-context-radius:\s*14px;[^}]*padding:\s*var\(--composer-context-pad\);[^}]*border-radius:\s*var\(--composer-context-radius\)/s,
    );
    expect(composerChromeCss).not.toMatch(
      /\.composer__chip-shell\s*\{[^}]*padding:\s*4px 10px/s,
    );
    expect(composerChromeCss).toMatch(
      /\.composer__context-item\s*\{[^}]*border-radius:\s*calc\(\s*var\(--composer-context-radius, 14px\)\s*-\s*var\(--composer-context-pad, 2px\)\s*\)/s,
    );
    expect(composerChromeCss).toMatch(
      /\.composer\s*\{[^}]*border-radius:\s*16px/s,
    );
  });
