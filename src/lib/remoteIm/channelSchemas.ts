...
/**
 * Channels hidden from the default sidebar/picker for this build (product
 * decision: this fork targets a non-domestic-China audience). Unlike
 * RETIRED_CHANNEL_IDS these stay fully implemented/bindable — only the
 * default listing is filtered, via `visibleChannelSchemas`.
 */
export const HIDDEN_CHANNEL_IDS: readonly RemoteChannelId[] = [
  "feishu",
  "lark",
  "dingtalk",
  "wecom",
...
/** CHANNEL_SCHEMAS minus HIDDEN_CHANNEL_IDS — use for any default channel listing/picker. */
export function visibleChannelSchemas(
  channels: readonly ChannelSchema[] = CHANNEL_SCHEMAS,
): ChannelSchema[] {
  return channels.filter(
    (c) => !(HIDDEN_CHANNEL_IDS as readonly string[]).includes(c.id),
  );
}
