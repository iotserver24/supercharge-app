 ...
 *
 * - **shared** (default): CLI home `~/.supercharge` (same sessions as terminal Supercharge CLI)
 * - **independent**: App agent-home `~/.supercharge-app/agent-home`
 *
...
export const SESSION_DATA_MODE_HOME = {
  independent: "~/.supercharge-app/agent-home",
  shared: "~/.supercharge",
} as const;
 ...
 * Shared `~/.supercharge` cannot see those sections, so the host keeps independent.
 */
