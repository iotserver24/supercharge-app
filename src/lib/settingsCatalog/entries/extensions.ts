import type { SettingsEntry } from "../types";

/** Settings catalog entries — extensions section. */
export const EXTENSIONS_ENTRIES: readonly SettingsEntry[] = [
  // ── extensions ──
  {
    id: "ext.plugins",
    section: "extensions",
    tab: "plugins",
    anchorId: "settings-anchor-ext-plugins",
    labelKey: "ext.plugins.title",
    descKeys: [
      "ext.lead",
      "ext.plugins.installLabel",
      "ext.plugins.recommendedTitle",
      "ext.plugins.recommended.xApiName",
      "ext.plugins.installableTitle",
    ],
    keywords: [
      "plugin",
      "plugins",
      "extensions",
      "chatcut",
      "codex",
      "x-api",
      "x api",
      "插件",
      "扩展",
      "擴展",
      "扩展中心",
      "擴展中心",
    ],
  },
  {
    id: "ext.plugins.installPath",
    section: "extensions",
    tab: "plugins",
    anchorId: "settings-anchor-ext-plugins-install",
    labelKey: "ext.plugins.advancedInstall",
    descKeys: [
      "ext.plugins.installLead",
      "ext.plugins.installHint",
      "ext.plugins.browseFolder",
    ],
    keywords: [
      "local path",
      "local folder",
      "install from path",
      "git",
      "owner/repo",
      "choose folder",
      "本地路径",
      "本地路徑",
      "本地文件夹",
      "从路径安装",
      "從路徑安裝",
    ],
  },
  {
    id: "ext.plugins.recommended",
    section: "extensions",
    tab: "plugins",
    anchorId: "settings-anchor-ext-plugins-recommended",
    labelKey: "ext.plugins.recommendedTitle",
    descKeys: ["ext.plugins.recommended.chatcutDesc"],
    keywords: ["chatcut", "codex", "recommended", "推荐", "推薦"],
  },
  {
    id: "ext.skills",
    section: "extensions",
    tab: "skills",
    anchorId: "settings-anchor-ext-skills",
    labelKey: "ext.skills.title",
    descKeys: ["ext.skills.edit", "ext.skills.editDisabledNote"],
    keywords: [
      "skill",
      "skills",
      "slash",
      "SKILL.md",
      "edit skill",
      "技能",
      "编辑技能",
    ],
  },
  {
    id: "ext.rules",
    section: "extensions",
    tab: "rules",
    anchorId: "settings-anchor-ext-rules",
    labelKey: "ext.rules.title",
    descKeys: ["ext.rules.lead"],
    keywords: [
      "rules",
      "AGENTS.md",
      "customize",
      "instructions",
      "规则",
      "規則",
      "项目规则",
    ],
  },
  {
    id: "ext.commands",
    section: "extensions",
    tab: "commands",
    anchorId: "settings-anchor-ext-commands",
    labelKey: "ext.commands.title",
    descKeys: ["ext.commands.lead"],
    keywords: [
      "commands",
      "slash command",
      "customize",
      "命令",
      "斜杠",
    ],
  },
  {
    id: "ext.skills.discoverExternal",
    section: "extensions",
    tab: "skills",
    anchorId: "settings-anchor-ext-skills-discover",
    labelKey: "ext.skills.discoverExternal",
    descKeys: [
      "ext.skills.discoverExternalDesc",
      "ext.skills.discoverExternalHonesty.configOff",
    ],
    keywords: [
      "claude",
      "cursor",
      "compat",
      "discover",
      "external skills",
      "探测",
      "探測",
    ],
  },
  {
    id: "ext.mcp",
    section: "extensions",
    tab: "mcp",
    anchorId: "settings-anchor-ext-mcp",
    labelKey: "ext.mcp.title",
    descKeys: [
      "ext.mcp.doctor",
      "ext.mcp.add",
      "mcpModal.oauth.authorize",
      "ext.mcp.auth.howToRefresh",
      "ext.mcp.pluginAuthLead",
    ],
    keywords: [
      "mcp",
      "model context protocol",
      "server",
      "oauth",
      "authorize",
      "x-api",
      "授权",
      "授權",
      "auth",
      "token",
      "credentials",
      "mcp doctor",
      "授权",
      "認證",
      "oauth 授权",
    ],
  },
  {
    id: "ext.agents",
    section: "extensions",
    tab: "agents",
    anchorId: "settings-anchor-ext-agents",
    labelKey: "ext.agents.title",
    descKeys: ["ext.agents.desc", "ext.agents.new"],
    keywords: [
      "agents",
      "agent definition",
      "persona",
      "scaffold",
      "new agent",
      "代理",
      "新建代理",
    ],
  },
  {
    id: "ext.hooks",
    section: "extensions",
    tab: "hooks",
    anchorId: "settings-anchor-ext-hooks",
    labelKey: "ext.hooks.title",
    keywords: ["hooks", "hook", "钩子", "鉤子", "生命周期钩子", "生命週期鉤子"],
  },
  {
    /** Legacy id kept for search; lands on plugins installable catalog (no market tab). */
    id: "ext.market",
    section: "extensions",
    tab: "plugins",
    anchorId: "settings-anchor-ext-plugins-catalog",
    labelKey: "ext.plugins.installableTitle",
    descKeys: ["ext.market.title", "ext.plugins.installableLead"],
    keywords: [
      "marketplace",
      "market",
      "install plugin",
      "市场",
      "市場",
      "市集",
      "openai",
      "openai/plugins",
      "catalog",
    ],
  },
];
