...
export function resolveToolPrimaryLabel(
  ...
  if (isSearchToolKind(tool.toolKind, tool.title, tool.toolCallId)) {
    const subject = toolInputDisplay(tool.input, "search") || toolPathBase(tool.path);
    return subject ? `${tr("chat.tool.search")} · ${subject}` : tr("chat.ranSearch");
  }
  ...
  let summary =
    bucket === "fallback"
      ? (tool.title && !/^(other|think|thinking)$/i.test(tool.title.trim())
          ? clip(tool.title)
          : humanizeToolKind(tool.toolKind === "other" ? null : tool.toolKind)) || tr(toolLabelKeyFor(tool.toolKind, bucket))
