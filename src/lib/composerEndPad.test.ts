...
describe("measureComposerEndPadPx", () => {
  ...
  it("includes bottom padding when the composer is portalled outside the stage", () => {
    const wrap = document.createElement("div");
    const stack = document.createElement("div");
    stack.className = "composer-stack";
    wrap.appendChild(stack);
    box(wrap, { top: 600, bottom: 800, height: 200 });
    box(stack, { top: 620, bottom: 784, height: 164 });
    expect(measureComposerEndPadPx(wrap)).toBe(180);
  });
