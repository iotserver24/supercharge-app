...
export const TimelineToolRow = memo(function TimelineToolRow({
  ...
  return (
    ...
    >
      ...
      <span className="activity-tool__status">
        {failed ? tr("activity.failed") : running ? tr("activity.running") : /^(completed|success|succeeded)$/.test(tool.status || "") ? tr("activity.done") : tool.status || ""}
      </span>
      {showBody && expand ? (
