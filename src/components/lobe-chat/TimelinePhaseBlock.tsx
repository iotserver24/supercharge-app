/** Compact activity overview with grouped tools and an opt-in chronological log. */
...
import { activityOverview } from "@/lib/activityOverview";
...
export const TimelinePhaseBlock = memo(function TimelinePhaseBlock({
  ...
  const overview = useMemo(() => activityOverview(phaseItems), [phaseItems]);
  const [showRaw, setShowRaw] = useState(false);
  const stepsResolved = useMemo(() => {
    if (!expanded || (!showRaw && !findQuery)) return [];
    return buildGrokActivitySteps(phaseItems, {
      live: phase.live,
      messageStreaming: !!messageStreaming,
    });
  }, [expanded, showRaw, findQuery, phaseItems, phase.live, messageStreaming]);
  const latestLabel = overview.latest
    ? resolveToolPrimaryLabel(overview.latest, (key, params) =>
        tr(key as MessageKey, params as Record<string, string> | undefined))
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="grok-act__header-text">
          {overview.failures > 0 && !phaseRunning ? tr("chat.activity.attention") : phaseChromeLabel}
        </span>
        <span className="activity-summary__counts">
          {overview.groups.filter((group) => group.kind !== "other").slice(0, 2).map((group) =>
            `${tr(`chat.activity.${group.kind}`)}: ${group.tools.length}`,
          ).join(" · ") || tr("chat.activity.actions", { n: overview.tools.length })}
        </span>
        <span className="activity-summary__toggle">
          {expanded ? tr("chat.activity.hide") : tr("chat.activity.show")}
        </span>
      ...
      {phaseItems.filter((item) => item.kind === "speech").map((item, index) => (
        <div className="grok-act__speech" key={`speech-${index}`}>
          <MarkdownChat locale={locale}>{item.text}</MarkdownChat>
        </div>
      ))}
      {phaseRunning && !expanded ? (
        <div className="activity-summary__latest" role="status">{latestLabel}</div>
      ) : null}
      {expanded ? (
        <>
          <ActivityDetails groups={overview.groups} locale={locale} live={phaseRunning} />
          <button type="button" className="activity-summary__raw" aria-expanded={showRaw || !!findQuery} onClick={() => setShowRaw(!showRaw)}>
            {tr("chat.activity.raw")}
          </button>
          {showRaw || findQuery ? (
            <GrokActivitySteps
              steps={stepsResolved}
